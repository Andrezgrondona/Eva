...
## ÁREAS DE ESPECIALIDAD
(…) [omitido para brevedad pero conservado en producción]
...
- Confirma datos antes de derivar a humano`,
  ...
  // 🧠 Flujo para agendamiento con 3 opciones + Google Meet
  let estadoAgendamiento = global.estadoAgendamiento || {};

  if (message.toLowerCase().includes("agendar cita") && !estadoAgendamiento.opcionesOfrecidas) {
    const ahora = new Date();
    const opciones = [
      new Date(ahora.getFullYear(), ahora.getMonth(), ahora.getDate() + 1, 10, 0),
      new Date(ahora.getFullYear(), ahora.getMonth(), ahora.getDate() + 1, 15, 0),
      new Date(ahora.getFullYear(), ahora.getMonth(), ahora.getDate() + 2, 11, 0),
    ];

    estadoAgendamiento = {
      opcionesOfrecidas: true,
      opciones,
    };
